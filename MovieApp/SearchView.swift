                    ...
                    ForEach(0 ..< items.count, id: \.self){idx in
                        HStack{
                            VStack{
                                ...
                                Text("id: \(items[idx].pq3), age\(items[idx].pq4), ImgNum\(items[idx].pq5)").frame(maxWidth: .infinity,alignment: .leading)
                                Text("protocol: \(items[idx].pq8), disease: \(items[idx].pq7)")
                            ...
                            Text("Load")  //Loadボタン
                                ...
                                .alert(isPresented:$showingAlert){
                                    Alert(title: Text("データをロードしますか？"), primaryButton:.default(Text("はい"),action:{
                                        ...
                                        user.age = Int(items[idx].pq4)!
                                        user.imageNum = Int(items[idx].pq5)!
                                        user.selected_hospital = user.hospitals.firstIndex(where: { $0 == items[idx].pq6})!
                                        user.selected_disease = user.diseases.firstIndex(where: { $0 == items[idx].pq7})!
                                        user.selected_movieType = user.movieTypes.firstIndex(where: { $0 == items[idx].pq8})!
                                        user.free_disease = items[idx].pq9
